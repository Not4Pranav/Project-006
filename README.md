...
> **Start here:** the complete, ordered setup and deployment checklist is in
> [SETUP.md](SETUP.md). This README is the architecture and reference guide.
...
[ bot.py  ·  SniperBot ]
            ...
            │                ├─ dnsrobot: loads https://dnsrobot.net/username-checker?u=<name>
            │                │  in isolated Chromium and reads the rendered Discord card
...
| Discord | 🐈‍⬛ | `dnsrobot` loads `https://dnsrobot.net/username-checker?u=<name>` in isolated Playwright/Chromium and reads the rendered Discord card; `account`/`account_api` use the configured account endpoint; `probe` uses its explicit URL | DNS Robot card says **Available** | DNS Robot card says **Taken** | page block, timeout, unknown/pending/rate-limit label, browser error, or malformed page |
...
`https://dnsrobot.net/username-checker` is a browser UI: its page loads
`?u=<name>`, runs its own browser-side check, and renders a Discord card.
`DISCORD_CHECK_MODE=dnsrobot` uses an isolated Playwright/Chromium context to
load that exact page and read the card; it does not pretend that an HTML GET
contains a result and does not substitute a direct server-side Discord POST.
The browser context receives no bot, account, or probe token. It also rejects
redirects away from DNS Robot before trusting page text.
...
├── test_checkers.py  # 34 offline checker tests + 2 optional LIVE=1 network tests
├── test_bot.py       # 30 end-to-end pipeline tests (simulated Discord messages)
├── .env.example      # copy to .env and fill in your secrets
├── requirements.txt  # runtime packages, including optional Playwright support
├── SETUP.md           # canonical ordered local + cloud setup/deployment guide
...
1. **Python 3.10 or newer** (the code uses modern type hints; check with `python --version`).
...
# If DISCORD_CHECK_MODE=dnsrobot, also run:
...
> Discord, HTTP, dotenv, and browser packages; an isolated `.venv` avoids
> conflicts with other projects. Chromium is installed separately only when
> the literal DNS Robot mode is enabled.
>
...
python test_checkers.py                 # 34 offline tests (+ 2 LIVE tests skipped)
...
left is the Discord token and intents described in [Phase 2](#phase-2--create-the-discord-application-do-this-once).
...
| `aiohttp>=3.9,<4` | Async HTTP clients for Minecraft, guns.lol, Account API, and probe checks |
| `python-dotenv>=1.0,<2` | Loads `.env` into environment variables at startup |
| `playwright>=1.48,<2` | Browser automation for the opt-in literal DNS Robot page mode |

### Common Phase 1 problems

| Problem | Fix |
| ------- | --- |
| `command not found: python` (macOS) | Use `python3`, or install Python 3.10+ from [python.org](https://www.python.org). |
...
| DNS Robot reports browser unavailable | Run `python -m playwright install chromium`; on Linux CI/VPS use `python -m playwright install --with-deps chromium`. |
...
| `DISCORD_CHECK_MODE` | — | `off` | `off`, `dnsrobot`, `account`, `account_api` (compatibility alias), or `probe` (case-insensitive) | `off` = skip Discord. `dnsrobot` = load DNS Robot in isolated Chromium and read its rendered card. `account` = POST the account API JSON contract. `probe` = query your own authorized checker URL. |
| `DISCORD_ACCOUNT_API_URL` | — | Discord first-party eligibility route | absolute HTTPS URL | Optional override for the account API; it must accept `{"username": "..."}` and return a strict boolean result. |
...
| `DISCORD_PROBE_URL` | — | blank | HTTPS URL template containing `{username}` | The external GET checker used with `DISCORD_CHECK_MODE=probe`; *never* defaults to `discord.com`. |
...
| `PROXY_URL` | — | direct | valid http/https proxy URL | Route all outbound checks through this proxy. Validated at startup; rejected before connecting if malformed. |
...
No DNS Robot token, account token, or extra URL is required. The adapter
loads the page in an isolated Playwright/Chromium context and reads the
rendered Discord card. Install the browser executable first:

```bash
python -m playwright install chromium
# Linux CI/VPS, if shared libraries are missing:
python -m playwright install --with-deps chromium
```

If Discord or DNS Robot blocks the browser, the bot reacts with ⚠️/a partial
result rather than treating the name as available.
...
```bash
python test_checkers.py      # 34 offline checker tests (+ 2 live tests skipped)
python test_bot.py           # 30 end-to-end pipeline tests
```
...
## Phase 5 — 24/7 hosting on Render (Background Worker)

A Discord bot only runs while its process is alive, so for always-on behavior
you run it on a host. Render's **Background Worker** is the matching service
for this repo; it is separate from Render's free Web Service tier. Railway and
any Python 3.10+ VPS use the same worker command.
...
| ---- | ------------ | ------------- | ----- |
| [Render](https://render.com) | Background Worker | `python bot.py` | Use a paid worker instance; install Chromium if using `dnsrobot` |
...
- **DNS Robot browser flow** — set `DISCORD_CHECK_MODE=dnsrobot` to load
  `https://dnsrobot.net/username-checker?u=<name>` in isolated Chromium and read
  the rendered Discord card. No DNS Robot credential is needed, but Chromium
  must be installed with `python -m playwright install chromium`. Page blocks,
  timeouts, and Discord rate limits remain unknown.
...
- **DNS Robot is a browser UI, not a DNS Robot server API.** Its published
  ...
  loads that page in isolated Chromium and reads the rendered card. A hosting
  IP blocked by DNS Robot or Discord still produces ⚠️/unknown, never FREE.
...
```bash
python test_checkers.py      # 34 offline tests (+ 2 skipped live tests) — should print OK
python test_bot.py           # 30 pipeline tests — should print OK
python checkers.py Notch           # live endpoints; Discord is skipped by default
python checkers.py vortex --mode dnsrobot  # literal DNS Robot Chromium page check
